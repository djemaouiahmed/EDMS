...
import {
  ...
  const commentRef = React.useRef([])
    ...
    const [usrArr, setUsrArr] = React.useState([])
     ...
     useEffect(() => {
    fetch('http://localhost:3000/MyDocumentsList',{method:'GET',credentials : "include" }).then(res=>res.json()).then(results =>{ 
     ...
     function setupuser(item) {
      ...
      setUsers(fetched)
        ...
        const [valueUsr, setValueUsr] = React.useState("")
...
async function downloaddoc (doc){
  ...
  <h1 className="text-3xl font-poppins mb-4 ">My Documents</h1>
  
  <div class="relative overflow-x-auto shadow-lg sm:rounded-xl w-[80%] left-24 top-12 bg-white">
